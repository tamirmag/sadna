...
public class AccountManager {
    ...
    public User getUser(String username) {
        return filterUsers(usernameFilter(username)).get(0);
    }
    ...
    public void logout(User u) throws AlreadyLoggedOut, UserNotExists {
        if (!users.contains(u)) throw new UserNotExists(u.getUsername());
    ...
    public UserManager login(String username, String password)
            throws UsernameNotValid, PasswordNotValid, UsernameAndPasswordNotMatch, AlreadyLoggedIn {
       /* Scanner c = new Scanner(System.in);
        ...
        ActionLogger.getInstance().writeToFile("User " + username + " successfully logged in.");*/
        if (username == null || username.equals("") || username.contains(" "))
            throw new UsernameNotValid(username);
        else if (password == null || password.equals("") || password.contains(" "))
            throw new PasswordNotValid(password == null? "null" : password);
    ...
    private boolean isPasswordCorrect(String username, String password) {
        return !filterUsers(passwordFitsToUsername(username, password)).isEmpty();
